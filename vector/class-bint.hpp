...
{
	if (p != nullptr) {
        std::cout<<"bint delete location: "<<(unsigned long)p<<std::endl;
        delete[] p;
        p = nullptr;
	}
	p = new int[len];
    std::cout<<"bint new location: "<<(unsigned long)p<<std::endl;
	if (p == nullptr) {
...
void Bint::_DoubleSpace()
{
    ...
    std::cout<<"bint delete location: "<<(unsigned long)data<<std::endl;
    delete[] data;
	data = newMem;
	capacity <<= 1;
}
